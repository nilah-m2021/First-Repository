...
let schoolName = "AppleBerry High"
let refrain = "Let's go \(schoolName) Let's go \(schoolName), We shoot for the sky, We are \(schoolName)."

let song = "\(refrain)\n\(refrain)\nYes, We are the champs that cannot be beat so all our opponents just take a seat. \(refrain) "
